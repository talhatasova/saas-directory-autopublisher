...
export class CopyGeneratorEngine {
  ...
  private static synthesizeDetailedReview(title: string, tagline: string, description: string, keywords?: string[]): string {
    ...
    const p2 = tagline ? `At its core, ${tagline.toLowerCase().replace(/\.$/, '')}. It eliminates repetitive manual effort and delivers high-impact results with minimal setup.` : `It delivers robust features and reliable infrastructure right out of the box.`;
    const p3 = description ? `Key capabilities include: ${description}` : `Users benefit from intuitive dashboards, real-time analytics, automated workflows, and instant collaboration features.`;
  ...
  private static classifyCategory(title: string, description: string, tags: string[]): string {
    ...
    if (combined.includes('ai') || combined.includes('gpt') || combined.includes('llm') || combined.includes('copilot')) {
      return 'AI Tools';
    }
    if (combined.includes('developer') || combined.includes('code') || combined.includes('api') || combined.includes('git')) {
      return 'Developer Tools';
    }
    if (combined.includes('analytic') || combined.includes('metric') || combined.includes('mrr') || combined.includes('revenue')) {
      return 'Analytics';
    }
    if (combined.includes('market') || combined.includes('seo') || combined.includes('email') || combined.includes('lead')) {
      return 'Marketing';
    }
    return 'Productivity';
  }
  ...
  private static classifyPricing(description: string, jsonLd?: any): 'free' | 'freemium' | 'paid' | 'subscription' | 'open_source' {
    if (jsonLd?.offers) {
      ...
      const hasFree = offers.some((o: any) => o.price === '0' || o.price === 0 || o.category?.toLowerCase() === 'freemium');
      const hasPaid = offers.some((o: any) => Number(o.price) > 0);
    ...
    if (lower.includes('freemium') || (lower.includes('free plan') && lower.includes('pro'))) return 'freemium';
    if (lower.includes('100% free') || lower.includes('completely free')) return 'free';
    if (lower.includes('/month') || lower.includes('subscription') || lower.includes('per month')) return 'subscription';
...
describe('Tier 1 Unit: Copy Generator & Enrichment Engine', () => {
  ...
  test('Classifies category accurately based on keywords', () => {
  ...
  test('Classifies pricing model from JSON-LD schema or copy', () => {
